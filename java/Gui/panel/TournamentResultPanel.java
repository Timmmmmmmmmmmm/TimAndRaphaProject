...
public class TournamentResultPanel extends JPanel {
    ...
    private List<PlayerTournamentInfoDto> players;
    ...
    private void showPlayerDetails(int index) {
        ...
        String games_played = DatabaseConnection.executeSql("SELECT COUNT(*) AS count FROM games g WHERE g.tournament_id = " + tournamentDto.id + " AND (g.player_white = " + playerId + " OR g.player_black = " + playerId + ");").getFirst().get("count");
        String wins = DatabaseConnection.executeSql("SELECT COUNT(*) AS count FROM games g WHERE g.tournament_id = " + tournamentDto.id + " AND ((g.player_white = " + playerId + " AND g.result = 1) OR (g.player_black = " + playerId + " AND g.result = -1));").getFirst().get("count");
        String losses = DatabaseConnection.executeSql("SELECT COUNT(*) AS count FROM games g WHERE g.tournament_id = " + tournamentDto.id + " AND ((g.player_white = " + playerId + " AND g.result = -1) OR (g.player_black = " + playerId + " AND g.result = 1));").getFirst().get("count");
        String draws = DatabaseConnection.executeSql("SELECT COUNT(*) AS count FROM games g WHERE g.tournament_id = " + tournamentDto.id + " AND g.result = 0 AND (g.player_white = " + playerId + " OR g.player_black = " + playerId + ");").getFirst().get("count");
